...
def get_rag_index_path(settings: Settings) -> Path:
    ...
    try:
        ...
        return (Path(base) / "multi-agent-course-hw8-faiss-index").resolve()
    return candidate
...
Правила:
1. Завжди починай з інструменту plan(request) — передай оригінальний або уточнений запит користувача одним рядком.
2. Виклич research(request) — передай план і контекст: що саме дослідити (можна включити пункти з ResearchPlan як текст).
3. Виклич critique(findings) — передай повний текст знахідок Research як один рядок.
4. Якщо verdict у результаті critique — REVISE, виклич research знову, додавши зворотний зв’язок (gaps, revision_requests). Максимум {max_revision_rounds} раундів доопрацювання після першого research.
