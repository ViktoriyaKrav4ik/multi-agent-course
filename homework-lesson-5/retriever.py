...
def index_ready() -> bool:
    ...
    # FAISS зберігає index.faiss + index.pkl (назва за замовчуванням "index")
...
def _build_retriever():
    ...
    if not index_ready():
        raise FileNotFoundError(
            f"Індекс не знайдено. Запустіть з папки homework-lesson-5: python ingest.py"
        )
...
def get_retriever():
    """Повертає кешований hybrid + rerank retriever."""
...
def hybrid_search(query: str) -> list:
    """Сирий список Document після retrieval (+ опційно reranking)."""
    settings = Settings()
