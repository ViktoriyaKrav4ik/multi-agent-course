"""
Ingest корпусу з corpus/ у FAISS + chunks для BM25.

Запуск з каталогу market-analyst: python ingest.py
"""
...
def ingest() -> None:
    ...
    if not (settings.rag_index_dir or os.environ.get("RAG_INDEX_DIR", "") or "").strip():
        try:
            ...
            print(
                "Шлях до проєкту містить не-ASCII; індекс за замовчуванням у temp "
                f"(див. get_rag_index_path). Збережено: {index_path}\n"
                "Або задайте RAG_INDEX_DIR у .env\n"
            )
    ...
    if not documents:
        print(
            "Документів не знайдено. Додайте PDF, TXT або MD у corpus/ "
            "і запустіть знову."
        )
        return
    ...
    print(f"FAISS індекс: {index_path}")
    print(f"Чанки BM25: {chunks_file}")
