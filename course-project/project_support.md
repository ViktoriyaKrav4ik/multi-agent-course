...
- **Месенджер:** Slack SDK, Telegram Bot API, або MCP-інтеграція
...
|-------|------------------------|----------------------|
| **Router** | Класифікує запит за категорією та терміновістю. Направляє до відповідного агента. | • LLM structured output — ClassificationOutput |
| **Docs Agent** | Обробляє запити по продукту: тарифи, функціонал, політика повернень, FAQ. Відповідає на основі внутрішньої бази знань. | • RAG — семантичний пошук по KB |
| **Web Search Agent** | Обробляє загальні/технічні запити, відповіді на які відсутні в KB: інтеграції, сумісність, зовнішні сервіси. | • DuckDuckGo Search — пошук у вебі |
| **Escalation Agent** | Обробляє критичні запити та випадки, де автоматична відповідь неможлива. Передає контекст людині-оператору через месенджер. | • Slack/Telegram API — нотифікація оператора · • File System tool — збереження escalation report |
